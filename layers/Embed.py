...
class PatchEmbedding_MutiPTST(nn.Module):
    def __init__(self, d_model, patch_len, stride, padding, dropout):
        super(PatchEmbedding_MutiPTST,self).__init__()
        # Patching
        self.patch_len = patch_len
        self.stride = stride
        self.padding_patch_layer = nn.ReplicationPad1d((0, padding))
...
class Embedding_forAMPTST(nn.Module):
    def __init__(self, input_dim, d_model, dropout):
        super(Embedding_forAMPTST, self).__init__()
        # Backbone, Input encoding: projection of feature vectors onto a d-dim vector space
        self.value_embedding = nn.Linear(input_dim, d_model, bias=False)
